...
"완료"가 결정론적 게이트로 검사되고 모든 실행이 결과를 기록하기 때문에, Yardlet은
벤치마크가 아니라 당신 자신의 이력으로부터 이 루프를 얼마나 신뢰할지 알려줄 수 있습니다.

`yardlet trust`는 실행 텔레메트리를 신뢰 뷰로 요약합니다.
...
이는 활성 인텐트로 스코프가 한정되므로, 인텐트를 가로질러 재사용된 task id의 시도가
한데 합쳐지지 않습니다. 읽기 전용입니다. 보고할 뿐, 스스로 라우팅이나 정책을 바꾸지
않습니다.
...
| `yardlet defer <id> [reason]` | 작업을 결정으로 따로 치워둠 (Deferred: 대기도 완료도 아님); 다시 큐에 넣어 되살림. |
| `yardlet handoff` | 최신 실행의 핸드오프 출력. |
| `yardlet report` | 인텐트의 최종 리포트 출력 (모든 작업의 집계). |
| `yardlet memory` | 프로젝트 메모리 인덱스 나열; `look_at` 랜드마크 대비 오래되었을 수 있는 문서를 표시. |
| `yardlet trust` | 신뢰 리포트: first-pass vs retried Done과 워커별 신뢰도 (읽기 전용). |
