...
| `R` | 리포트/히스토리 브라우저. |
| `m` | Monitor 열기: 선택한 Failed/Partial 작업의 최신 실패 시도를 확인하거나 워커 실시간 출력을 따라가기. |
| `s` | 설정 (실행 중에도 열 수 있음). |
...
Failed 또는 Partial 작업을 선택하고 `m`을 누르면 현재 intent의 해당 task channel에서
최신 실패 시도를 엽니다. Monitor는 워커를 재실행하지 않고 subprocess 종료 메타데이터와
정확한 `stdout.log`, `stderr.log` 경로 및 내용을 보여 줍니다. 빈 파일은 빈 스트림으로
표시합니다. 파일을 읽을 수 없으면 내용을 만들어 내지 않고 정확한 경로와 읽기 오류를
보여 줍니다. 다른 행에서는 기존 live Run Monitor를 계속 따라갑니다.

새 작업, 플랜 수정, Answer 요청 입력 상자에서는 다음 키를 사용합니다.

| 키 | 동작 |
| --- | --- |
| `Enter` | 줄바꿈 삽입. Shift 또는 Alt 구분에 의존하지 않습니다. |
| `Ctrl+S` | 모든 지원 터미널에서 요청 전송. 빈 입력은 전송하지 않습니다. |
| `Ctrl+Enter` | 터미널이 keyboard enhancement를 지원해 이 조합을 구분할 때 전송. |
| `Home` / `End` | 현재 줄의 처음 / 끝으로 이동. |
| `↑` / `↓` / `←` / `→` | 줄 사이와 줄 안에서 caret 이동. |
| `Backspace` / `Delete` | caret 앞 / 위치의 문자 삭제. |
| `Esc` | 취소하고 입력 초안을 버림. |

Answer 상자에서는 `PgUp` / `PgDn`으로 초안 위의 읽기 전용 문맥을 추가로 스크롤합니다.
...
코어가 staging 결과를 가져온 serial run에만 Yardlet은 durable 내부
`yardlet-txn/...` branch에서 native `git commit`으로 격리 commit을 만듭니다. 따라서
저장소 commit hook과 `commit.gpgSign`이 그대로 적용됩니다. commit의 정확한 parent와
평가 시점에 고정한 tree를 검증한 다음, 그 commit만 compare-and-swap으로 run-owned
branch에 게시합니다. hook 또는 서명 실패 시 target branch를 바꾸지 않고 worktree를
보존하며, concurrent ref 변경도 fail-closed합니다. parallel worker는 이 transaction
record를 제공하거나 재개할 수 없고 marker 없는 immutable-commit 경로를 유지합니다.
merge 뒤 재시작 cleanup은 기록된 worker commit과 계속 일치하는 worktree 및 ref만
제거합니다. 현재 branch를 검사하는 commit hook은 serial 완료 중 내부 transaction
branch를 보게 됩니다.

serial 경로의 provenance와 integration, cleanup, no-change, Git-finish 권한은 worker가
쓸 수 있는 run directory 밖의 core-owned receipt에 저장됩니다. recovery는 손상되거나
다른 대상으로 바뀐 run projection을 이 receipt로 재구성하고, 원격 검증된 finish도
queue와 봉인된 run projection이 모두 일치한 뒤에만 terminal로 취급합니다. 따라서
완료된 worker를 다시 실행하거나 이미 적용된 push를 반복하지 않고 cleanup과 push의
모든 crash window를 닫습니다.
...
호환은 명시적으로 분리된 branch입니다. 실제 legacy event는 request digest가 없을 수 있고,
replay는 synthetic duplicate를 추가하거나 기존 journal을 변경하지 않습니다. event journal은 seq가 1부터 연속이고, filename과
...
lock 획득은 bounded timeout을 둔 non-blocking 방식이며 interrupt와 contention error를
...
저장합니다. activated queue는 confirmation parity용 immutable materialized base plan을
보존합니다. confirmed task의 identity, 상대 순서, worker, scope, dependency, acceptance는
그대로 유지되고 typed scheduler metadata만 변할 수 있습니다. 명시적 user add와 worker
follow-up ingest는 confirmation materialization marker가 비어 있는 append-only runtime task이며,
각각 별도의 core-owned immutable origin receipt와 실행 계약이 정확히 일치해야 합니다. committed
ordinal marker는 이 task들의 존재와 순서를 append-only로 고정합니다. hard `runs_before` edge와
stale decision capability clear는 exact typed runtime receipt를 immutable base에 재생할 수 있을
때만 허용됩니다. receipt 준비는 이 marker가 생기기 전까지 provisional입니다. queue CAS가 effect를
노출하지 못했다면 well-formed uncommitted receipt는 이후 retry가 안전하게 대체할 수 있습니다.
effect가 queue에 들어간 뒤에는 exact after-queue digest만 누락 commit marker를 복구할 수 있습니다.
confirm 이후 skill projection은 새로 ingest한 task에만 적용됩니다.
Snapshot, status, TUI startup, packet, approval, queue, run, add, finalization, recovery는 변조된
envelope나 누락 receipt를 trusted work로 노출하거나 canonical byte를 바꾸기 전에 거절합니다.
각 express goal은 session create, proposal, accept, confirm 전체에서 같은 outer
...
.agents/
  ...
  work-queue.yaml           작업
  planning.lock             kernel이 보유하는 workspace mutation transaction lock
  planning-sessions/        session, immutable proposal/draft, ordered event, action receipt
  activations/              committed exact-promotion receipt
  runtime-task-receipts/    confirm 이후 user/follow-up task의 immutable origin
  runtime-capability-receipts/ typed stale-decision capability migration
  activation-required.yaml fail-closed scheduling을 위한 durable V010-origin discriminator
  *-policy.yaml             도구 / 승인 / 상호작용 / 리서치 / 빌링 정책
  ...
  runs/<run-id>/            실행별 산출물 (결과, 검증, 체크포인트, 핸드오프, git-finish projection)
  conversations/<id>.yaml   워커에게 다시 이어지는 needs-user 대화 기록
  checkpoints/              권위 복구 receipt + 최신 컴팩트 재개 지점
