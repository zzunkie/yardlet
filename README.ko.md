...
않습니다. snapshot, activation, completed action receipt 중 하나라도 맞지 않으면 실행 불가로
남습니다. planning mutation은 하나의 workspace kernel lock을 사용합니다. immutable
revision과 event는 덮어쓰지 않는 atomic create로 만들고, session과 action 전이는
compare-and-swap으로 저장합니다. 모든 terminal action receipt는 typed effect event를
연결합니다. 새 confirm은 Queued, Running, NeedsUser, Partial, Blocked 작업이 남은 active
queue를 교체하지 않으며, 손상된 activation guard를 inactive workspace로 취급하지 않고
오류로 반환합니다.
...
.agents/
  ...
  work-queue.yaml           작업
  planning.lock             kernel이 보유하는 workspace planning transaction lock
