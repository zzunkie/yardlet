...
조사 기준일: 2026-07-12
정정: 2026-07-12 (YARD-005) — 독립 review(`docs/reviews/builtin-skill-library-review.md`) F-001 반영.
alias mapping(1.1절) 추가, 전체 인용 경로 존재 재확인, LE-020·LE-021 관찰 사실 정정.
...
- 본문의 repo 표기는 공개용 alias다. 실제 로컬 이름은 tracked 문서에 기록하지 않는다. 1.1절의
  fingerprint를 `<workspace>` 바로 아래 후보에 적용해 alias를 결정적으로 해석한 뒤 인용 경로를
  `test -e`로 확인한다. 실제 alias-to-path 해석 결과는 로컬 run validation에만 남긴다.

아래 표에서 `관찰 사실`은 파일에서 직접 확인한 사실이고 `inferred need`는 built-in library에 대한 해석이다. 둘을 같은 문장에 섞지 않는다.

### 1.1 Alias mapping (경로 재현용, YARD-005 추가)
...
|---|---|
| `yard` | `Cargo.toml`, `src/`, `templates/agents/`, `.github/workflows/ci.yml` |
| `workspace-erp-monorepo` | `docs/erp-rebuild/`, `pnpm-workspace.yaml` |
| `workspace-secondary-monorepo` | `packages/server/test/app.e2e-spec.ts`, `packages/erp/src/tests/App.test.tsx` |
| `workspace-agent-runtime` | `docs/map/README.md`, `pyproject.toml`, `app/agent/` |
| `brain` | `go.mod`, `internal/core/engine_test.go`, `internal/memory/file_test.go` |
| `workspace-backend-builder` | `apps/executor/`, `docker-compose.e2e.yml` |
| `workspace-service` | `docs/codebase-map.ko.md`, `src/domains/` |
| `workspace-content-pipeline` | `docs/cardnews-image-pipeline.md`, `docs/operator-guide.md` |
| `workspace-quant-system` | `docs/runbooks/paper-bot-operations.md`, `tests/backtest/` |
| `workspace-gitops` | `charts/app-template/Chart.yaml`, `charts/app-template/templates/` |
| `workspace-knowledge-base` | `CONVENTIONS.md`, `templates/linear/` |
| `workspace-book-publisher` | `main/skills/book-builder/`, `main/renderer/README.md` |
| `workspace-desktop-media` | `internal/sprite/`, `frontend/package.json`, `go.mod` |
| `workspace-mobile-companion` | `apps/relay/src/ws/`, `apps/pwa/src/` |
| `workspace-mobile-app-a` | `package.json`, `README.md`, React Native dependency set, fingerprint A recorded in local validation |
| `workspace-mobile-app-b` | `package.json`, `README.md`, React Native dependency set, fingerprint B recorded in local validation |
| `workspace-game` | `project.godot`, `docs/asset-production-spec-20260622.md` |
| `workspace-mcp-server` | `docs/verify-connection.png`, MCP server manifest |
| `workspace-next-app` | `package.json` with Next.js dependency and no stronger fingerprint above |
| `workspace-vite-app` | `package.json` with Vite and i18next dependencies |
| `workspace-poc-kit` | `docs/bootstrap.md`, `docs/connectors.md`, `docs/secrets.md` |
| `workspace-interactive-web` | `docs/tactics-and-simulation.md`, `docs/data-model.md` |
| `workspace-client-app` | `docs/adapter-strategy.md`, `docs/platform-research.md` |
| `local-reference-catalog` | `catalog/skills.tsv`, `presets/core.skills`, no repository metadata |
| `workspace-web-platform` | nested checkout with `.claude/skills/agent-browser`, `docs/testing/` |

Worktree variants are excluded by resolving `.git` files through their `gitdir:` target and retaining only the
primary checkout for each repository.
...
| LE-015 | `workspace-mobile-app-a/package.json`, `workspace-mobile-app-a/README.md`, `workspace-mobile-app-b/package.json`, `workspace-mobile-app-b/README.md` | 두 repo 모두 React Native Android/iOS 앱이고 build, lint, test, native setup script가 있다. | native mobile preset, platform build/release overlay 수요 | 강함. release나 signing은 approval과 secret이 필요한 별도 overlay다. |
...
| LE-019 | `workspace-poc-kit/README.md`, `workspace-poc-kit/docs/bootstrap.md`, `workspace-poc-kit/docs/connectors.md`, `workspace-poc-kit/docs/secrets.md` | runtime code를 복제하지 않는 배포 scaffold이며 bootstrap, connector, hosting, secret 문서를 가진다. | deployment scaffold preset, connector and secret-boundary overlay 수요 | 강함. 실제 customer runtime으로 분류하지 않는다. |
| LE-020 | `workspace-interactive-web/package.json`, `workspace-interactive-web/docs/data-sources.md`, `workspace-interactive-web/docs/data-model.md`, `workspace-interactive-web/docs/tactics-and-simulation.md`, `workspace-interactive-web/docs/tech-stack.md` | Next.js 앱 manifest(`package.json`의 next/react 의존과 build/dev/test/typecheck script, `next.config.ts`, `app/`, `src/`)와 product/data/simulation research 문서가 함께 tracked되어 있다. | web-ui 신호와 research/planning 수요가 공존. research 문서는 source evaluation overlay 수요 근거 | 중간. 초판의 "코드 manifest 없음" 관찰은 오류였고 YARD-005에서 정정했다. 문서 인상만으로 preset을 정하지 말고 manifest를 교차 확인해야 하는 사례다. |
| LE-021 | `workspace-client-app/package.json`, `workspace-client-app/docs/adapter-strategy.md`, `workspace-client-app/docs/platform-research.md` | TypeScript CLI 자동화 도구 manifest(typecheck/test/cli script, playwright-core·vitest·tsx 의존)와 adapter/platform research 문서가 함께 tracked되어 있다. | research/planning 수요와 browser-automation 도구 수요가 공존 | 중간. 초판의 "코드 manifest 없음" 관찰은 오류였고 YARD-005에서 정정했다. 두 문서만으로 product preset을 자동 선택하면 안 된다는 제한은 유지한다. |
...
| WF-014 | `workspace-mobile-app-a/package.json`, `workspace-mobile-app-b/package.json`, `workspace-mobile-companion/apps/relay/src/ws/` | native platform build, device permissions, realtime relay가 일반 web build와 다른 검증을 요구한다. | native-mobile 또는 realtime overlay | Product preset + overlay |
...
| PRESET-D6 | docs/templates/manuscript/renderer 중심 | LE-011, LE-012 | structured authoring, citation, artifact packaging | manifest가 없으면 구현 preset을 추정하지 않는다. |
...
`workspace-interactive-web`와 `workspace-client-app`은 초판에서 PRESET-D6 근거로 묶었으나, YARD-005
재검증에서 둘 다 code manifest가 확인되어(LE-020, LE-021 정정) docs-only 분류 근거에서 제외했다. 두 repo는
오히려 문서 인상만으로 preset을 정하면 안 되고 manifest 교차 확인이 선행해야 한다는 분류 원칙(6절 1·2항)의
사례다. research 문서 자체의 수요는 OVERLAY-D9로만 유지한다. PRESET-D7은 workspace에서 수요가 분명하지만 Godot, React Native, Wails의 toolchain이 크게 다르므로 하나의 거대한 preset보다 별도 소형 preset이 적합하다는 제한을 둔다.
...
| OVERLAY-D9 | Research and citation | source comparison, research ledger, research 문서 중심 task | local code diagnosis만 필요한 작업 | LE-020, LE-021의 research 문서군, `workspace-book-publisher/main/books/ai-token-to-agent/docs/research-ledger.md` |
...
2. **Path와 script 교차 확인**: package name이나 repo name만 보지 말고 bin/source path, workspace packages, test script, framework dependency를 함께 본다. LE-014는 `mobile`이라는 이름만으로 native mobile로 분류하면 실패하는 반례고, LE-020/LE-021은 문서 인상이 실재하는 manifest를 가리는 역방향 반례다(YARD-005 정정).
...
5. **Negative signal과 no-match 보존**: code manifest가 실제로 없는 repo, scaffold README, provenance 없는 local catalog는 억지 분류하지 않는다. core만 적용하고 research 또는 사용자 선택으로 확장할 수 있어야 한다. 단 "문서 중심으로 보인다"는 인상은 negative signal이 아니다. LE-020/LE-021처럼 research 문서가 앞서 보여도 manifest가 있으면 manifest 신호가 성립한다.
...
- 초판의 alias 경로는 mapping이 없어 재현 불가능했다(YARD-004 review F-001). YARD-005에서 1.1절
  mapping을 추가하고 전체 인용 경로의 존재 확인과 관찰 표본 재검증(OC 수치, AGENTS/README 내용 표본)을
  다시 수행했으며, 이 과정에서 LE-020·LE-021의 manifest 관찰 오류를 정정하고 PRESET-D6, OVERLAY-D9,
  6절 결론에 반영했다.
...
| 모든 인용 경로가 실제 경로 또는 추적 가능한 alias mapping으로 재현 | Pass (YARD-005) | 1.1절 mapping + `test -e` 108개 전부 통과, 관찰 표본 재검증 포함 |
| 반복 작업을 tracked 안정 근거로 입증하고 운영 이력 mining 제외 | Pass | 1절 조사 규칙과 각 WF path. run/telemetry/conversation 내용은 사용하지 않음 |
| `local-reference-catalog`를 비복제 비교 증거로만 사용 | Pass | OC-001부터 OC-005와 비복제 판정. OC 수치는 YARD-005에서 실측 재확인(90 skills, 22 presets, core 7, symlink target, git/LICENSE 부재) |
| Core, product preset, overlay 수요를 구분하고 약한 추론 제한 | Pass | 4.1부터 4.3, LE-018부터 LE-021의 제한(LE-020·LE-021은 정정 반영), 7절 |
