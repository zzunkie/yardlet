...
**NEXT SESSION:** implement the two reworks above (review = drop hard dep + keep
2-cap loop; auto-commit = serial-in-worktree or worktree-only) + the targeted
bugs, then a 5th review to confirm convergence, then release v0.8.0.

## Status

- **Branch** `v0.8-finalize-run`, **PUSHED to origin** through `3054bfb`. v0.8
  ...
  `3054bfb`) fixed ~38 review findings. **128 tests green**, fmt + clippy `-D
  warnings` clean.
- **v0.8.0 NOT released** (no main merge / crates.io / GitHub release yet). Two
  reworks gate the release confidence: review-auto-remediation (drop the hard
  `depends_on`, keep the 2-cap loop) and auto-commit (serial-in-worktree, or
  worktree-only) — see the hardening-review section above. Resume there.
