...
## Human-decision vs missing-capability (2026-07-01)

Slice 1b resolved only the *alias* question ("no alias table; an out-of-vocab
`required_capabilities` entry == park as Blocked") and left the rest "undecided".
The *decision-vs-capability* distinction was never decided or built, and it got
lost. A dogfood run surfaced what that omission costs (workspace specifics live in
local session memory, not in this committed doc).

**Symptom.** A worker-proposed follow-up that is really a *human creative choice*
(pick option A vs B) parked `Blocked` at queue creation — "no enabled worker
declares required capability [...]" — with no clean resolver (only `defer`, or
hand-adding the capability to a worker). Yet the SAME class of work, when a
running worker hits it mid-task, is handled cleanly: the worker asks via
`question_for_user` (NeedsUser) and the user resolves it with `yardlet answer`
(including a "explain it more simply" round-trip).

**Root cause.** Yardlet has no first-class concept separating:
  - a **human decision / approval** — ask the user, resolved by `yardlet answer`
    via the NeedsUser flow — from
  - a **missing capability** — needs a tool / license / new worker, where Blocked
    is the correct dead-end until a worker is added.
The framing actively CONFLATES them: the packet calls user-decisions
"gates/ceilings" and groups a "user-decision ceiling" with a "license/tool
ceiling", and the planner prompt (`planner.rs` capability block) tells the model
to express "an external or human decision" as an off-vocab capability. A worker
proposing a FOLLOW-UP has only `required_capabilities` to signal "this needs
something special" — there is no "needs a human decision: <question>" field. So a
human-decision follow-up gets shoehorned into a fake off-vocab capability and
`reconcile_queue_capabilities` parks it Blocked at ingest. Outcome is
non-deterministic: the same kind of decision lands in NeedsUser (written in prose,
hit at run time) or Blocked (written as a fake capability) by LLM whim. The worker
is not misbehaving — it faithfully follows a framing that treats decisions and
tool-gaps as one species.

**Decision (closes the leftover 1b nuance).**
1. A human decision / choice / approval is NOT a capability. Express it as a
   QUESTION → NeedsUser, resolved by `yardlet answer`. Reserve
   `required_capabilities` strictly for a worker's tool / skill / license need
   (where Blocked-until-a-new-worker is the right gate). This does NOT reopen the
   alias decision: routing stays a dumb structural gate; the semantic split is
   the planner's job (identity.md: judgment in the planner, not the core).
2. Planner + worker packet: stop instructing the model to "name a capability for
   a human decision". Instruct instead: a human decision → a question
   (`questions_for_user` / a follow-up needs-decision field); capabilities are
   tools-a-worker-needs only.
3. Follow-up schema: add a way to propose a future human-decision task that
   ingests as NeedsUser with the question seeded, so a worker can hand a decision
   to the user cleanly (today its only lever is a capability → Blocked). Minimal
   complement: even with no new field, a decision follow-up left with NO
   capability would just run and ask its question at run time — so step 2 alone
   already removes the dead-end.

A live regression case is parked in local session memory for post-fix validation.
