...
**NEXT:** 5th review to confirm convergence, then release v0.8.0.
...
- **Branch** `v0.8-finalize-run`, **PUSHED to origin** through `ccb813d`. v0.8
  ...
  rework above) is staged locally but **not yet committed/pushed**. **127 tests
  green** (the obsolete `git_commit_worker_changes` test was dropped with the
  function; the 1c test now asserts soft-priority ordering), fmt + clippy `-D
  warnings` clean.
- **v0.8.0 NOT released** (no main merge / crates.io / GitHub release yet). The
  two release-gating reworks are now IMPLEMENTED (review-auto-remediation = soft
  priority, no hard dep; auto-commit = worktree-only interim) — see the
  hardening-review section above. Remaining gate: a 5th review to confirm
  convergence, then release.
