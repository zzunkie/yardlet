...
- **v0.8 main** (after foundation): Project Memory MVP (2-4), Trust Report (5),
  ...
  - **Refinement (next): intent-scoped telemetry.** `RunTelemetry` has no
    `intent_id`, so a task id reused across intents folds its attempts together
    (e.g. a 2-week-old workspace shows "YARD-005 16 attempts" summed across
    intents). Add `intent_id` to `RunTelemetry` + set it from `queue.intent_id`
    in `finalize_run`'s telemetry block; then `trust` can group/scope by intent.
    Legacy records lack it (default ""), so it only sharpens going forward.
...
- **Tier-2 per-slice**: stale detection (git-diff on landmark `look_at` paths),
  ...
  source~~ (DECIDED = telemetry; see Trust Report MVP above — `intent_id` on
  `RunTelemetry` is the open refinement), mining structured-validation
...
- **Branch** `v0.8-finalize-run`. Foundation slices 1–1d landed (`33950d1` →
  `e55f554`); then `e9620b2` (run-record seal, 1e) and `5351352` (Trust Report
  MVP, 5). **121 tests green**, fmt + clippy `-D warnings` clean. The two newest
  feature commits + their docs are **LOCAL, not pushed**.
