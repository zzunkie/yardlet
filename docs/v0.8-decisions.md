...
- **Slice 1e** — close the gaps the finalize map revealed: parallel runs no
  ...
  - **Parallel-path validation — DONE** (commit `6f78257`): `finalize_run` now
    ...
    test instead.)
  - Still open in 1e: recovery telemetry/handoff; finalize-time auto-push.
...
- **Branch** `v0.8-finalize-run`. Foundation slices 1–1d landed (`33950d1` →
  ...
  5), `e93e003` (intent-scoped trust, 5), and `6f78257` (parallel worktree
  validation, 1e). **123 tests green**, fmt + clippy `-D warnings` clean. The
  2026-06-23 feature commits + their docs are **LOCAL, not pushed**.
