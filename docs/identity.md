# Identity — what Yardlet is, in one place

> **Rent the intelligence. Own the work.**
> (지능은 빌리고, 작업은 소유한다.)

Yardlet is a local console for **engineering the loop** that turns a few
...
worker contract: packet in → process → result files out. A Yardlet-shipped
...
of Yardlet's features are the logistics that a standard unit makes possible.
...
Yardlet gets *stronger* when workers improve, because its product is not
...
Work done in a chat session belongs to no one — it evaporates. Yardlet keeps
...
accountability compounds. Yardlet sits on the compounding side.
...
Yardlet is that practice as a product, with one addition the vendor loop
...
Vendor loops run inside one vendor's walls; Yardlet is the loop you own across
all of them.
...
- Positioning: *Workers race on capability; Yardlet races on nothing-gets-lost.*
...
writer (workers propose, Yardlet records) and an eval loop self-corrects, so
