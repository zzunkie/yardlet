...
- 보수 판정: terminal proposal, undo linkage, stripped provenance, stable prepared effect, immutable journal 검증, completed-active 일치, bounded process lock, runtime queue 경쟁, receipt/session/runtime envelope P0를 포함한 결정적 process test 30개가 통과했다. V010-002 최종 완료 표시는 독립 재검토 뒤에만 갱신한다.
...
process fixture는 run과 confirm, add가 경쟁할 때 stale confirm은 거절되고 runtime state는
보존되며 activated queue membership을 바꾸려는 두 add는 동일 envelope 오류로 거절됨을
확인한다. activated queue는 confirmed 당시 immutable `materialized_queue`를 provenance
digest 경계로 보존하고, 별도 runtime task state만 변경하므로 exact draft parity와
runnability가 동시에 유지된다.
...
activated runtime queue는 current task의 id와 order를 고정하고 title, priority, risk, kind,
worker/model/effort, dependency, skill/capability, scope, acceptance, goal/validation/approval/
interaction, rationale/provenance를 materialized queue와 비교한다. process fixture는 title,
scope, worker, risk 변조에서 `exact_active_parity: false`를 확인한 뒤 `queue`, `run`, `add`,
`defer`, `resolve`, `recover`가 모두 `active_runtime_envelope_mismatch`로 실패하고 canonical
byte가 유지됨을 검사했다. `Partial`에서 `Done`으로의 state-only resolve는 parity를 유지한다.
...
- v2 terminal action receipt의 effect id/type/digest/exact payload는 immutable journal event의
  canonical byte와 digest에 일치해야 하며 v1은 별도 compatibility branch다.
- persisted session의 journal directory와 session path/latest pointer identity는 필수다.
- activated current queue는 materialized queue와 id/order 및 모든 task 실행 field가 같고
  `Task.state`만 달라질 수 있다. 변조 시 queue/run/add/finalize/recovery가 active byte를
  바꾸지 않고 `active_runtime_envelope_mismatch`로 거절한다.
