...
pub struct L {
    ...
    /// Shown when Enter is pressed on an approval-pending task: Enter must never
...
pub const EN: L = L {
    ...
    footer_home: "\u{2191}\u{2193} select  Enter next-action  n new  r run  A auto  m monitor  h handoff  i goal  f access  s settings  l lang  g refresh  R reports  q quit",
...
pub const KO: L = L {
    ...
    press_a: "  (a 키)",
    see_handoff: "핸드오프 참고",
    footer_home: "\u{2191}\u{2193} 선택  Enter 다음행동  n 새작업  r 실행  A 자동  m 모니터  h 핸드오프  i 목표  f 권한  s 설정  l 언어  g 새로고침  R 보고  q 종료",
